...
import {
  Brain, UserRound, Plus, Pencil, Check, X, Loader2,
  Users, ChevronLeft, ArrowRight,
} from "lucide-react";
...
export default function TeamManagePage() {
  ...
  async function loadTeam() {
    ...
    try {
      ...
      setAgents(scopeRes.data.data.filter((a) => !HIDDEN_ROLES.has(a.role)));
      setAbigailCeo(orgRes.data.data.find((a) => a.role === "global-ceo") ?? null);
    } catch {
  ...
  async function sendInterview(userText: string) {
    ...
    if (count >= 3) {
      setMessages((prev) => [
        ...prev,
        { role: "abigail", content: `Perfect — I have everything I need. Let me define this agent now…` },
      ]);
  ...
  if (view === "interview") {
    return (
      <div className="flex flex-col h-full">
        ...
        {interviewPhase === "done" && (
          <div className="shrink-0 px-4 py-4 max-w-2xl mx-auto w-full">
            <Button onClick={() => setView("team")} className="w-full h-11 rounded-2xl text-[14px] font-medium">
              Back to team
            </Button>
          </div>
        )}
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="shrink-0 px-6 pt-8 pb-5 flex items-center justify-between gap-4 border-b border-border/40">
        <div className="space-y-0.5">
          <div className="flex items-center gap-2">
            <Users className="size-4 text-muted-foreground" />
            <h1 className="text-[18px] font-semibold tracking-tight">Manage Team</h1>
          </div>
          <p className="text-[13px] text-muted-foreground">
            Rename agents, review their roles, or hire new specialists via Abigail interview.
          </p>
        </div>
        <Button
          onClick={openInterview}
          className="h-9 px-4 rounded-xl text-[13px] font-medium shrink-0"
        >
          ...
          <>
            ...
            {agents.map((agent) => (
              <AgentCard
                ...
                renaming={renaming}
                onStartRename={() => { setRenamingId(agent.id); setRenameValue(agent.name); }}
            ...
            {agents.length === 0 && !abigailCeo && (
              <div className="text-center py-16 text-muted-foreground text-[14px]">
                No agents found. Add your first specialist above.
              </div>
            )}
...
function AgentCard({
  ...
  return (
    <div className={cn("rounded-2xl border bg-gradient-to-br p-4 group", colorClass)}>
      <div className="flex items-center gap-3">
        <div className="size-10 rounded-xl bg-background/30 flex items-center justify-center shrink-0">
          <UserRound className="size-4 text-foreground/50" />
        </div>
