...
const SUGGESTIONS = [
  { label: "Define research scope", icon: Sparkles },
  { label: "Identify data sources", icon: Wand2 },
  { label: "Set success criteria", icon: Lightbulb },
];

export default function ResearchWorkflowProjectPage({
  params,
}: {
  ...
  React.useEffect(() => {
    ...
    const current = activeProjects.research;
    if (current?.id === projectId) return;
    listProjects()
      .then((res) => {
        const match = (res.data.data as { id: string; name: string; description?: string | null; team: string }[])
          .find((p) => p.id === projectId && p.team === "research");
        if (match) setActiveProject("research", match.id, match.name, match.description);
      })
      .catch(() => {});
  }, [isHydrated, isAuthenticated, projectId, activeProjects.research, setActiveProject]);

  const { messages, send, sendWithContext, sendHidden, isLoading, historyLoaded, activeSessionId, toggleExpanded } = useWorkflowChat("research");
  ...
  const project = activeProjects?.research;

  // Subscribe to live canvas data at page level so it survives view toggles.
  const rawCanvasData = useCanvasLiveData(projectId, "research");
  ...
  const subtitle = project
    ? "Chat with your Research Team to define scope and objectives."
    : "Loading project…";
