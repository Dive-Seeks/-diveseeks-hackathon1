...
import { z } from "zod";
import {
  Loader2, Lock, Mail, User, Eye, EyeOff, Code, Briefcase,
...
export default function RegisterPage() {
  ...
  const form = useForm<RegisterFormValues>({
    ...
    defaultValues: {
      ...
      isCoder: false, isBusiness: false,
    },
  });
  ...
  async function onSubmit(values: RegisterFormValues) {
    ...
    try {
      await registerMutation.mutateAsync({
        ...
        isCoder: selectedRole === "coder",
        isBusiness: selectedRole === "business",
      });
      toast.success("Account created! Check your email to verify.");
      router.push("/login");
    } catch (error: unknown) {
      const message = extractApiError(error).message || "Registration failed. Please try again.";
      toast.error(message);
  ...
  return (
    <div className="min-h-screen bg-[#030712] text-slate-200 relative overflow-hidden flex items-center">
      ...
      {a && <div className={`absolute inset-0 bg-gradient-to-r ${a.glow} to-transparent transition-all duration-700 pointer-events-none`} />}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-violet-500/4 rounded-full blur-3xl pointer-events-none" />
        ...
        <div className="flex flex-col lg:flex-row lg:items-stretch gap-6 lg:gap-8">
          ...
          <div className="flex flex-col gap-5 lg:w-[42%] lg:flex-shrink-0">
            ...
            <div>
              ...
              <h1 className="text-2xl sm:text-3xl font-bold text-white leading-[1.15] mb-1.5 font-[family-name:var(--font-display)] tracking-[-0.0125em]">
                ...
                <span className="block text-zinc-600 font-semibold mt-0.5">
                  Choose your role<br />to get started.
                </span>
              </h1>
              <p className="text-zinc-500 text-sm">Select how you will use DiveSeeks.</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {/* Coder */}
              <button
                onClick={() => router.push("/register?role=coder")}
              ...
              {/* Business */}
              <button
                onClick={() => router.push("/register?role=business")}
                className={`group relative flex flex-col p-4 rounded-xl border text-left transition-all duration-300 overflow-hidden bg-zinc-950/70 ${
            ...
            <div className="flex items-center gap-4">
              {[{ icon: Shield, label: "Encrypted" }, { icon: Zap, label: "60s setup" }, { icon: Brain, label: "Zero GPU" }].map(({ icon: Icon, label }) => (
            ...
            <p className="text-xs text-zinc-600 -mt-1">
              ...
              <button onClick={() => router.push("/login")} className="text-zinc-400 hover:text-white transition-colors font-medium underline underline-offset-4">
                Sign in
              </button>
            </p>
          </div>
            ...
            {!selectedRole ? (
              ...
              <div className="p-6 sm:p-7">
                <div className="mb-5">
                  ...
                  <h2 className="text-2xl font-bold text-white mb-1 font-[family-name:var(--font-display)] tracking-[-0.0125em]">Create account</h2>
                  <p className="text-zinc-500 text-sm">
                    {isCoder ? "Set up your AI coding workspace" : "Launch your business dashboard"}
                  </p>
                </div>
                  ...
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3.5">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-[10px] uppercase tracking-[0.18em] text-zinc-500 font-medium font-[family-name:var(--font-display)]">First</FormLabel>
                            <FormControl>
                              <div className="relative">
                                <User className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-zinc-600 pointer-events-none" />
                                <Input placeholder="John" className={`pl-9 h-10 bg-zinc-900/80 border-zinc-700/60 text-white placeholder:text-zinc-700 rounded-xl text-sm transition-all ${a!.inputFocus}`} {...field} />
                              </div>
                            </FormControl>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-[10px] uppercase tracking-[0.18em] text-zinc-500 font-medium font-[family-name:var(--font-display)]">Last</FormLabel>
                            <FormControl>
                              <div className="relative">
                                <User className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-zinc-600 pointer-events-none" />
                                <Input placeholder="Doe" className={`pl-9 h-10 bg-zinc-900/80 border-zinc-700/60 text-white placeholder:text-zinc-700 rounded-xl text-sm transition-all ${a!.inputFocus}`} {...field} />
                              </div>
                            </FormControl>
                            <FormMessage className="text-xs" />
                          </FormItem>
                        ...
                        <FormItem>
                          ...
                          <FormControl>
                            <div className="relative">
                              ...
                              <Input placeholder="you@company.com" className={`pl-10 h-10 bg-zinc-900/80 border-zinc-700/60 text-white placeholder:text-zinc-700 rounded-xl text-sm transition-all ${a!.inputFocus}`} {...field} />
                            </div>
                          </FormControl>
                          <FormMessage className="text-xs" />
                        </FormItem>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <div className="relative">
                                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-zinc-600 pointer-events-none" />
                                <Input type={showPassword ? "text" : "password"} placeholder="Min 8 chars" className={`pl-9 pr-9 h-10 bg-zinc-900/80 border-zinc-700/60 text-white placeholder:text-zinc-700 rounded-xl text-sm transition-all ${a!.inputFocus}`} {...field} />
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-[10px] uppercase tracking-[0.18em] text-zinc-500 font-medium font-[family-name:var(--font-display)]">Confirm</FormLabel>
                            <FormControl>
                              <div className="relative">
                                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-zinc-600 pointer-events-none" />
                                <Input type={showConfirmPassword ? "text" : "password"} placeholder="Repeat" className={`pl-9 pr-9 h-10 bg-zinc-900/80 border-zinc-700/60 text-white placeholder:text-zinc-700 rounded-xl text-sm transition-all ${a!.inputFocus}`} {...field} />
                                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-600 hover:text-zinc-400 transition-colors">
                    ...
                    <button
                      type="submit"
                      disabled={isLoading}
                      className={`w-full h-10 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 transition-all duration-300 mt-1 ${a!.button}`}
                    >
                      {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <>Create account <ChevronRight className="w-4 h-4" /></>}
                    </button>
                  </form>
                </Form>
