...
@Injectable()
export class MerchandisingManagerService extends BaseManagerService {
  constructor(soulEngine: SoulEngine, providerRouter: AiProviderRouter) {
    super(soulEngine, providerRouter);
  }

  async reviewMerchandising(
    output: unknown,
    title: string,
    constraints?: unknown,
  ) {
    return this.review('merchandising_mgr', output, title, constraints);
  }
}
