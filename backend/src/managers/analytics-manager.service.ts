...
@Injectable()
export class AnalyticsManagerService extends BaseManagerService {
  constructor(soulEngine: SoulEngine, providerRouter: AiProviderRouter) {
    super(soulEngine, providerRouter);
  }

  async reviewAnalytics(output: unknown, title: string, constraints?: unknown) {
    return this.review('analytics', output, title, constraints);
  }
}
